...
public class GameBoardView implements ActionListener {
	...
	private void buildBezierCurve(Point2D start, Point2D end, double angle) {
		...
		// P1
		double x = start.getX() + d * Math.cos(angle);
		double y = start.getY() + d * Math.sin(angle);
		...
		BezierCurve curve = new BezierCurve("TrajectoryPreview", start, end, p1, p2, angle, finalAngle);
	...
	private void buildOutData() {
		...
		Point2D initialPosition = path.getTrajectoryStart();
		...
		Point2D[] cp = path.getTrajectoryControlPoints();
		if (cp.length != 2) {
			// not a bezier curve with 2 control points
			return;
		}

		double cp1Distance = cp[0].distance(initialPosition);
		double cp2Distance = cp[1].distance(finalPosition);

		trajectoryOutData = new BezierMoveOutData(x, y, finalAngle, cp1Distance, cp2Distance);
	}
