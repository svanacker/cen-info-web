...
public abstract class AbstractNavigationMap implements INavigationMap {
	...
	public void addSplinePath(String location1, String location2, int cpDistance1, int cpDistance2, double angle1, double angle2) {
		...
		addVector(new PathVectorSpline(start, end, cpDistance1, cpDistance2, angle1, angle2));
	}
	...
	public Location getNearestLocation(Point2D position) {
		Location p = new Location("tmp", (int) position.getX(), (int) position.getY());
	...
	@Override
	public void updateWeights(Point2D position, double radius, int additionalWeight, int maxWeight) {
		Location l = new Location("tmp", (int) position.getX(), (int) position.getY());
		for (IPathVector v : vectors) {
