...
public class SonarArea extends AModelisableRobotAttribute{
	...
	public SonarArea(Properties pProperties){
		angle = PropertiesUtils.getSize(pProperties, KEY_SONAR_ANGlE).getWidth();
		angle = Math.toRadians(angle)/2 + Math.PI/2;
		portee = Double.parseDouble((String)pProperties.get(KEY_SONAR_SCOPE));
		relativePosition = (PropertiesUtils.getVector(pProperties, KEY_SONAR_POSITION));
	...
	protected void  computeRelativeCorners() {
		upLeft = new Point2D.Double(relativePosition.y + portee * Math.sin(angle), relativePosition.x - portee * Math.cos(angle));
		upRight = new Point2D.Double(relativePosition.y + portee * Math.sin(angle), relativePosition.x + portee * Math.cos(angle));
		bottomLeft = new Point2D.Double(relativePosition.y , relativePosition.x  );
		bottomRight = new Point2D.Double(relativePosition.y , relativePosition.x );
	}
}
