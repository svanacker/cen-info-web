...
public abstract class ARobotAttributeElement extends AMovableElement {
	...
	public ARobotAttributeElement(String NAME,
			AModelisableRobotAttribute pModelisableRobotArea,
			ARobotElement pRobotElement) {
		super(NAME, pModelisableRobotArea.getRelativeCentralPoint(),
				pRobotElement.getOrientation());
		robotElement = pRobotElement;
		position = relativeToAbsolute(pRobotElement,
				pModelisableRobotArea.getRelativeCentralPoint());
	...
	private Point2D relativeToAbsolute(ARobotElement robotElement,
			Point2D pRelative) {
		...
		Point2D absolute = new Point2D.Double(d * Math.cos(a) + cp.getX(), d
				* Math.sin(a) + cp.getY());
		return absolute;
	}
}
