...
public class SimulMoving extends Thread {
	...
	private double distanceRefresh;

	private double distanceTotal;

	private List<IGameBoardElement> elements;

	private Rectangle2D gamePlayBound;

	private String handler;

	private IMovableElement movedElement;

	private ISimulMovingHandler movingHandler;
	...
	private void firePosition(PositionStatus status) {
		...
		movingHandler.onSimulMoveEvent(new AbsolutePositionEvent(x, y, alpha,
				status, handler));
	}
	...
	public void run() {
		...
		do {
			try {
				...
				// orientation
				currentPosition = (Point2D.Double) movedElement
				.getPosition().clone();
				currentAlpha = movedElement.getOrientation();
				RobotPosition newPosition = new RobotPosition(
						currentPosition.getX(), currentPosition.getY(),
						currentAlpha);

				//TODO calculer le ratioMove en fonction du temps r��l d'attente du Thread qui peut �tre plus long que sleep
				...
				deltaCurve.setResults(deltaAlpha, radius, deltaDistance,
						direction);
				...
				remainingCurve.setResults(remainingCurve.getAngle() - deltaAlpha, radius, remainingCurve.getDistance() - deltaDistance, direction);
	...
	private void transform(Point2D point, double angle, Point2D translation) {
		...
		double x = point.getX();
		double y = point.getY();
		point.setLocation(x * c - y * s + translation.getX(), x * s + y * c
				+ translation.getY());
	}
