...
public class CoordinatesTest2011 {
	...
	private void initializeWebCam() {
		InputStream is = getClass().getClassLoader().getResourceAsStream("org/cen/cup/cup2011/robot/robot2011.properties");
	...
	private void printConfiguration(Graphics g, List<Region> regions) {
		...
		for (PawnPosition position : positions) {
			...
			int sx = (int) sp.getX();
			int sy = (int) sp.getY();
			System.out.println("Coordinates (" + x + ", " + y + ") -> (" + sx + ", " + sy + ") : " + isElementPresent(g, regions, sx, sy));
		}
	}
	...
	private void showValues() {
		String text = String.format("orientation : %.2f\ncolor : %.2f\nSeuil : %.2f\nInclinaison WebCam : %.2f\nAngle de vision : %.2f\nxPos : %.2f\nyPos : %.2f\nzPos : %.2f\nFiltre couleur (ms) : %d\nFiltre régions (ms) : %d", Math.toDegrees(zAngle), colorAngle, regionThreshold, Math.toDegrees(cameraInclination), Math.toDegrees(cameraVisionAngle), xPos, yPos, zPos, colorFilterTime, regionsFilterTime);
		if (textArea != null) {
