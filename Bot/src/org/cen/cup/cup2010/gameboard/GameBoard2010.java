...
public class GameBoard2010 extends AbstractGameBoard {
	...
	private void addElements() {
		...
		elements.add(new StartArea(StartAreaColor.YELLOW, new Point2D.Double(0, BOARD_HEIGHT - StartArea.START_AREA_HEIGHT)));
		elements.add(new Border("border", BOARD_WIDTH + BORDER_WIDTH, new Point2D.Double(-BORDER_WIDTH, -BORDER_WIDTH), 0));
		elements.add(new Border("border", BOARD_WIDTH + BORDER_WIDTH, new Point2D.Double(-BORDER_WIDTH, BOARD_HEIGHT), 0));
		...
		for (int y = CORN_START_Y; y <= BOARD_MIDDLE_HEIGHT; y += CORN_SPACING_Y) {
			...
			for (int x = CORN_START_X; x < BOARD_WIDTH; x += CORN_SPACING_X) {
				if ((x > (CORN_START_X + CORN_SPACING_X * 4) && y == CORN_START_Y) || (x == CORN_START_X && y > CORN_START_Y) || (x == (CORN_START_X + CORN_SPACING_X) && y > (CORN_START_Y + CORN_SPACING_Y))) {
				...
				if (corn) {
					elements.add(new Corn("corn" + cornId, new Point(x, y), false));
					if (y < BOARD_MIDDLE_HEIGHT) {
						elements.add(new Corn("corn" + cornId, new Point(x, BOARD_HEIGHT - y), false));
					}
					cornId++;
				} else {
					elements.add(new Tomato("tomato" + tomatoId, new Point(x, y)));
					if (y < BOARD_MIDDLE_HEIGHT) {
						elements.add(new Tomato("tomato" + tomatoId, new Point(x, BOARD_HEIGHT - y)));
					}
					tomatoId++;
				}
				corn = !corn;
