...
public class NavigationHandler2010 extends AbstractDeviceHandler {
	...
	private void addCommands(List<RobotDeviceRequest> requests, TrajectoryPathElement pathElement, Point2D centralPoint, Set<TomatoControlPoint2010> tomatoes) {
			...
			if (cp instanceof TomatoControlPoint2010) {
				...
				if (tcp.isFinalPoint()) {
					...
					if (r instanceof MoveRequest) {
						...
						requests.remove(n - 1);
						double distance = mr.getDistance() - DISTANCE_FINAL_POINT;
						...
						if (currentPathElementsIndex < currentPathElements.size()) {
							currentPathElements.remove(currentPathElementsIndex);
						}
					}
				}
				...
				if (r instanceof MoveRequest) {
					...
					requests.add(new CollectCorn2010Request(side, org.cen.cup.cup2010.device.specific2010.CollectCorn2010Request.Action.SEQUENCE));
					// // Test du maïs
	...
	private void addCornControlPoint(int index, String location, String... segments) {
		corns[index].setName(location);
		Location l = getMapLocation(location);
		CornControlPoint2010 cp = new CornControlPoint2010(l.getPosition(), "", corns[index]);
	...
	private void addTomatoControlPoint(int index, String location, boolean finalPoint) {
		Location l = getMapLocation(location);
		if (l == null) {
		...
		TomatoControlPoint2010 cp = new TomatoControlPoint2010(l.getPosition(), "", tomatoes[index], finalPoint);
		...
		for (PathSegment segment : currentObjective.getSegments()) {
			for (ControlPoint controlPoint : segment.getControlPoints()) {
				if (controlPoint.isEnabled()) {
					...
					controlPoints.add(p);
					gameBoard.getElements().add(new ControlPointMarker(PROPERTY_CONTROLPOINTS, p));
				}
			}
		}
	...
	private void computeNextRequests() {
		if (currentPathElements == null || currentPathElementsIndex >= currentPathElements.size()) {
			trajectoryDone();
			return;
		}
		...
		while (true) {
			...
			if ((cp == null || !cp.mustStop() || !cp.isEnabled()) && isNextPointAligned(centralPoint, p)) {
				continue;
			}
	...
	private List<TrajectoryPathElement> getSortedControlPoints(Location start, PathSegment segment) {
		...
		for (ControlPoint cp : segment.getControlPoints()) {
			...
			if (!Double.isNaN(p.getX()) && !Double.isNaN(p.getY())) {
				list.add(new TrajectoryPathElement(p, cp, segment.getStart(), segment.getEnd()));
			}
		}
		Collections.sort(list, new TrajectoryPathElementComparator(start.getPosition()));
		return list;
	}
	...
	private List<TrajectoryPathElement> getTrajectoryPath(List<Location> path) {
		...
		for (Location l : path) {
			if (start != null) {
				if (result.isEmpty()) {
					result.add(new TrajectoryPathElement(start.getPosition(), null, start, l));
				}
				if (!goingToGoal) {
					PathSegment s = findSegment(start, l);
					if (s != null) {
						List<TrajectoryPathElement> list = getSortedControlPoints(start, s);
				...
				result.add(new TrajectoryPathElement(l.getPosition(), null, start, l));
			}
			start = l;
		}
		return result;
	...
	private void initialize() {
		...
		addCornControlPoint(6, "R4C2", "R2C2,R4C1", "R4C1,R6C2", "R6C2,R4C3", "R4C3,R2C2");
		addCornControlPoint(7, "R4C4", "R2C4,R4C3", "R4C3,R6C4", "R6C4,R4C5");
		addCornControlPoint(8, "R6C3", "R4C3,R6C2", "R6C2,R8C3", "R8C3,R6C4", "R6C4,R4C3");
		addCornControlPoint(9, "R6C5", "R4C5,R6C4", "R6C4,R8C5");
		addCornControlPoint(10, "R8C2", "R6C2,R8C1", "R8C1,R10C2", "R10C2,R8C3", "R8C3,R6C2");
		addCornControlPoint(11, "R8C4", "R8C5,R6C4", "R6C4,R8C3", "R8C3,R10C4");
		addCornControlPoint(12, "R10C1", "R10C0,R8C1", "R8C1,R10C2", "R10C2,R12C1");
		addCornControlPoint(13, "R10C3", "R10C4,R8C3", "R8C3,R10C2", "R10C2,R12C3");
	...
	private void initializeLocationGroups() {
		...
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("blue central", blueGoal * 2), "R2C0", "R4C1", "R6C2", "R8C3", "R10C4", "blue end");
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("blue left", 0), "R10C2", "R12C3");
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("blue right", yellowGoal), "R0C1", "R2C2", "R4C3", "R6C4", "R8C5");
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("yellow central", yellowGoal * 2), "R10C0", "R8C1", "R6C2", "R4C3", "R2C4", "yellow end");
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("yellow right", 0), "R2C2", "R0C3");
		strategyHandler.addLocationGroup(pathSegments, new LocationGroup2010("yellow left", blueGoal), "R12C1", "R10C2", "R8C3", "R6C4", "R4C5");
	...
	private boolean isNextPointAligned(Point2D o, Point2D p1) {
		...
		TrajectoryPathElement pathElement = currentPathElements.get(currentPathElementsIndex);
		Point2D p2 = pathElement.getPosition();
	...
	private void selectLocationGroup() {
		...
		RobotPosition position = RobotUtils.getRobotAttribute(RobotPosition.class, servicesProvider);
		Point2D centralPoint = position.getCentralPoint();
		List<LocationGroupStats> results = strategyHandler.analyzeGroups(centralPoint, position.getAlpha());
	...
	public void setProperties(Properties properties) {
		...
		String s = properties.getProperty(PROPERTY_ASYNCHRONOUS, PROPERTY_ASYNCHRONOUS_DEFAULT_VALUE);
	...
	private boolean waitForResult(RobotDeviceRequest r) {
		if (r instanceof CollectTomato2010Request || r instanceof Specific2010SleepRequest || r instanceof WaitPositionUpdateRequest) {
			return false;
		}
		return true;
	}
