...
public abstract class AbstractGameActionService implements IGameActionService {
    ...
    @Override
    public List<IRobotDeviceRequest> buildNextRequests(TrajectoryData data, IGameActionMap map) {
        List<IRobotDeviceRequest> requests = new ArrayList<IRobotDeviceRequest>();
        List<TrajectoryActionData> actionsData = new ArrayList<TrajectoryActionData>();
        ...
        while (pe.isExtendable()) {
            ...
            if ((end == null && pe.getStart().distance(p) < DISTANCE_MINIMUM_FROM_LAST)
                    || (end != null && p.distance(end) < DISTANCE_MINIMUM_FROM_LAST)) {
    ...
    protected abstract void handleAction(List<IRobotDeviceRequest> requests, Point2D position, IGameAction action,
            IGameActionHandler handler);

    private boolean handleActions(List<TrajectoryActionData> actionsData, IGameAction action, TrajectoryPathElement pe,
            Collection<IGameActionHandler> handlers) {
        if (handlers != null) {
    ...
    protected abstract double handleMove(List<IRobotDeviceRequest> requests, Point2D start, Point2D end,
            double orientation);
