...
public class GameBoardPainter {
     ...
     */
    public Point2D getRealCoordinates(Point2D screenCoordinates) {
        Point2D realCoordinates = new Point2D.Double();
        transformInv.transform(AwtAdapterUtils.toAwtPoint2D(screenCoordinates),
                AwtAdapterUtils.toAwtPoint2D(realCoordinates));
    ...
    private void paintElement(IGameBoardElement e, Graphics2D g2d, AffineTransform t) {
        ...
        java.awt.geom.Point2D p = transform
                .transform(AwtAdapterUtils.toAwtPoint2D(point), (java.awt.geom.Point2D) null);
    ...
    private void paintNavigationPoint(AffineTransform transform, Graphics2D g) {
        ...
        if (getDrawFlags().contains(GameBoardFlags.LABELS)) {
            ...
            java.awt.geom.Point2D point = new java.awt.geom.Point2D.Float();
            for (Location location : locations) {
                Font font = g.getFont().deriveFont(10f);
                g.setFont(font);
                Point2D lpoint = location.getPosition();
                point = t.transform(AwtAdapterUtils.toAwtPoint2D(lpoint), point);
                // Affichage du nom
