...
public class CoordinatesTest2011 {
	...
	public static final int COLUMN_OFFSET_LAST_X = COLUMN_OFFSET_X
			+ COLUMN_SPACING_X * (COLUMNS_X_COUNT - 1);

	public static final int COLUMN_OFFSET_LAST_Y = COLUMN_OFFSET_Y
			+ COLUMN_SPACING_Y * (COLUMNS_Y_COUNT - 1);
	...
	private void addAngleLabel(Container cp, GridBagConstraints gbc) {
		...
		angleSlider = new JSlider(0, 36000,
				(int) (Math.toDegrees(cameraVisionAngle) * 100));
		...
		angleSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				angleSlider.setToolTipText(Double.toString(Math
						.toDegrees(cameraVisionAngle)));
				updateWebCam();
				drawSource();
			}
		});
	...
	private void addColorFilterControl(Container cp, GridBagConstraints gbc) {
		...
		colorSlider = new JSlider(0, 36000,
				(int) (Math.toDegrees(colorAngle) * 100));
	...
	private void addInclinationControl(Container cp, GridBagConstraints gbc) {
		...
		inclinaisonSlider = new JSlider(0, 9000,
				(int) (Math.toDegrees(cameraInclination) * 100));
		gbc.gridwidth = GridBagConstraints.REMAINDER;
		...
		inclinaisonSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				inclinaisonSlider.setToolTipText(Double.toString(Math
						.toDegrees(cameraInclination)));
				updateWebCam();
				drawSource();
				showValues();
			}
	...
	private void addOrientationControl(Container cp, GridBagConstraints gbc) {
		...
		orientationSlider = new JSlider(-360, 360,
				(int) (Math.toDegrees(zAngle)));
		...
		orientationSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				orientationSlider.setToolTipText(Double.toString(Math
						.toDegrees(zAngle)));
	...
	private void addRegionThresholdControl(Container cp, GridBagConstraints gbc) {
		...
		slider.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				regionThreshold = (double) slider.getValue();
				buildRegionFilter();
				showValues();
			}
		});
	...
	private void buildRegionFilter() {
		...
		for (int i = 1; i <= n; i++) {
			...
			g.drawLine((int) p.getX() - 2, (int) p.getY(), (int) p.getX() + 2,
					(int) p.getY());
			g.drawLine((int) p.getX(), (int) p.getY() - 2, (int) p.getX(),
					(int) p.getY() + 2);
	...
	private void drawSource() {
		...
		BufferedImage bi = new BufferedImage(im1.getWidth(null),
				im1.getHeight(null), BufferedImage.TYPE_INT_ARGB);
	...
	private void initGUI() {
		...
		image1.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				Point2D p = c.screenToAbsolute(robotPosition, e.getX(),
						e.getY());
	...
	private void initializeWebCam() {
		...
		robotPosition.setFromProperties(properties,
				Strategy2009.PROPERTY_INITIAL_POSITION + ".BLUE.");
	...
	private boolean isElementPresent(Graphics g, List<Region> regions, int cx,
			int cy) {
	...
	private void saveImage(Image im) {
		try {
			ImageIO.write((RenderedImage) im, "png", new File(
					"C:\\Users\\Emmanuel\\Documents\\image.png"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void setSource(String name) {
		...
		URL u = l.getResource("org/cen/cup/cup2011/gameboard/configuration/"
				+ name);
		Image image = Toolkit.getDefaultToolkit().getImage(u);
		image1.setIcon(new ImageIcon(image));
		im1 = new BufferedImage(image.getWidth(null), image.getHeight(null),
	...
	private void showValues() {
		String text = String
				.format("orientation : %.2f\ncolor : %.2f\nSeuil : %.2f\nInclinaison WebCam : %.2f\nAngle de vision : %.2f\nxPos : %.2f\nyPos : %.2f\nzPos : %.2f\nFiltre couleur (ms) : %d\nFiltre régions (ms) : %d",
						Math.toDegrees(zAngle), colorAngle, regionThreshold,
						Math.toDegrees(cameraInclination),
						Math.toDegrees(cameraVisionAngle), xPos, yPos, zPos,
						colorFilterTime, regionsFilterTime);
		if (textArea != null) {
	...
	private void updateWebCam() {
		...
		wcp.setImageDimension(new Dimension(im1.getWidth(null), im1
				.getHeight(null)));
	}
}
