...
public class GameBoard2010 extends AbstractGameBoard {
    ...
    private final ArrayList<IGameBoardElement> elements;
    ...
    private void addElements() {
        ...
        elements.add(new StartArea(StartAreaColor.BLUE, new Point2D.Double(0, 0)));
        elements.add(new StartArea(StartAreaColor.YELLOW, new Point2D.Double(0, BOARD_HEIGHT
                - StartArea.START_AREA_HEIGHT)));
        elements.add(new Border("border", BOARD_WIDTH + BORDER_WIDTH, new Point2D.Double(-BORDER_WIDTH, -BORDER_WIDTH),
                0));
        elements.add(new Border("border", BOARD_WIDTH + BORDER_WIDTH, new Point2D.Double(-BORDER_WIDTH, BOARD_HEIGHT),
                0));
        elements.add(new Border("border", BOARD_HEIGHT, new Point2D.Double(-BORDER_WIDTH, BOARD_HEIGHT), -Math.PI / 2));
        elements.add(new Border("border", 2000, new Point2D.Double(BOARD_WIDTH, 500), Math.PI / 2));
        elements.add(new Hill("hill", new Point2D.Double(0, 670)));
        elements.add(new Border("hill border", 1519, new Point2D.Double(500 + BORDER_WIDTH, 740), Math.PI / 2));
        ...
        for (int y = CORN_START_Y; y <= BOARD_MIDDLE_HEIGHT; y += CORN_SPACING_Y) {
            ...
            for (int x = CORN_START_X; x < BOARD_WIDTH; x += CORN_SPACING_X) {
                ...
                if (corn) {
                    elements.add(new Corn("corn" + cornId, new Point2D.Double(x, y), false));
                    if (y < BOARD_MIDDLE_HEIGHT) {
                        elements.add(new Corn("corn" + cornId, new Point2D.Double(x, BOARD_HEIGHT - y), false));
                    }
                    cornId++;
                } else {
                    elements.add(new Tomato("tomato" + tomatoId, new Point2D.Double(x, y)));
                    if (y < BOARD_MIDDLE_HEIGHT) {
                        elements.add(new Tomato("tomato" + tomatoId, new Point2D.Double(x, BOARD_HEIGHT - y)));
                    }
                    tomatoId++;
                }
                corn = !corn;
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < ox.length; i++) {
                ...
                elements.add(new Orange("orange" + orange, new Point2D.Double(x, y)));
                orange++;
            }
        }
    }
